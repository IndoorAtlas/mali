...
exports.getMethodDescriptorsProto = function (serviceFullName, service) {
...
function isService(obj) {
  return typeof obj === 'function' && typeof obj.service === 'object'
}

exports.getServices = function (proto) {
  const services = {}
  function pickServices(prefix, object) {
    Object.getOwnPropertyNames(object).forEach(key => {
      if (key === 'nested' || key === 'parent' || key === 'google') {
        return
      }
      const name = `${prefix}${key}`
      const nested = object[key]
      if (isService(nested)) {
        services[name] = nested
      } else if (_.isObject(nested)) {
        pickServices(`${name}.`, nested)
      }
    })
  }
  pickServices('', proto)
