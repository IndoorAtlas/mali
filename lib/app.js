...
class Mali extends Emitter {
  ...
  addService (path, name, options) {
    ...
    if (load) {
      ...
      const services = mu.getServices(proto)
      _.forOwn(services, (service, serviceName) => {
        this.services[serviceName] = service.service
        this.middleware[serviceName] = []
        this.methods[serviceName] = mu.getMethodDescriptorsProto(serviceName, service.service)
      })
    }
